...
def _build_vocab(annotations, threshold=1):
    ...
    vocab=list(zip(*(counter.most_common(9996)))[0])
    # print ('Filtered %d words to %d words with word count threshold %d.' % (len(counter), len(vocab), threshold))
    print ('Filtered %d words to %d words' % (len(counter), len(vocab)))
...
def main():
    ...
    # about 5000 images and 25000 captions for val / test dataset
    val_cutoff = int(0.125 * len(val_dataset))
    test_cutoff = int(0.25 * len(val_dataset))
